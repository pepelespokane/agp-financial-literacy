...
(function () {
  ...
  // ── Load questions ──────────────────────────────────────────────────────
  // Try the per-game file first (questions-{gameId}.json), then fall back to
  // the generic questions.json so older single-game deployments still work.
  async function loadQuestions() {
    const perGame = `questions-${gameId}.json`;
    let resp = await fetch(perGame);
    if (!resp.ok) {
      resp = await fetch('questions.json');
    }
